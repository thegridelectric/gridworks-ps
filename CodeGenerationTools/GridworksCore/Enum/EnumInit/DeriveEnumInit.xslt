<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="/">
        <FileSet>
            ...
            <FileSetFile>
                    <xsl:element name="RelativePath"><xsl:text>../../../../src/gwprice/enums/__init__.py</xsl:text></xsl:element>

                <OverwriteMode>Always</OverwriteMode>
                <xsl:element name="FileContents">
<xsl:text>""" Enums used in gwprice """
...
# From gwprice</xsl:text>
<xsl:for-each select="$airtable//ProtocolEnums/ProtocolEnum[(normalize-space(ProtocolName) ='gwprice')]">
<xsl:sort select="LocalEnumName" data-type="text"/>
<xsl:variable name="enum-id" select="Enum"/>
<xsl:for-each select="$airtable//GtEnums/GtEnum[GtEnumId=$enum-id]">
<xsl:text>
...
__all__ = [</xsl:text>
<xsl:for-each select="$airtable//ProtocolEnums/ProtocolEnum[(normalize-space(ProtocolName) ='gwprice')  or (normalize-space(ProtocolName) ='gridworks')]">
<xsl:sort select="LocalEnumName" data-type="text"/>
<xsl:variable name="enum-id" select="Enum"/>
<xsl:for-each select="$airtable//GtEnums/GtEnum[GtEnumId=$enum-id]">
